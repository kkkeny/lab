...
Calendar.prototype.draw = function() {
	...
	_cs[_cs.length] = ' <tr>';
	_cs[_cs.length] = '  <th colspan="200"><input type="button" class="b" name="closeButton" id="closeButton" \/><\/th>';
	_cs[_cs.length] = ' <\/tr>';
	...
	var control_date = document.getElementById('control_date');
};
	...
	for(var i = 0; i < tds.length; i++) {
		...
		if (dateArray[i]){
			tds[i].onclick = function () {
				...
				var calendarDate = calendar.date.getFullYear()+'-'+calendar.date.getMonth()+1+'-'+this.innerHTML;
				var date1 = new Date(calendarDate).getTime();
				if(control_date!==null){
					if(date1<new Date(nowYear+'-'+nowMonth+'-'+nowDate).getTime()){
						sessionStorage.setItem('dateColor',this.index);
						...
						calendar.hide();
							sessionStorage.setItem('now',calendar.dateControl.value);
								window.location.href = "/day/"+calendar.dateControl.value;
					}else{
						return;
					}
			...
			if (today.getFullYear() == calendar.date.getFullYear()) {
				if (today.getMonth() == calendar.date.getMonth()) {
					if ((today.getDate()-1) == dateArray[i]) {
						// tds[i].style.backgroundColor = calendar.colors["bg_cur_day"];
						tds[i].onmouseover = function () {this.style.backgroundColor = calendar.colors["bg_out"];}
						// tds[i].onmouseout  = function () {this.style.backgroundColor = calendar.colors["bg_cur_day"];}
						if(sessionStorage.getItem('dateColor')){
							tds[sessionStorage.getItem('dateColor')].style.backgroundColor = calendar.colors["bg_cur_day"];
						}else{
							tds[i].style.backgroundColor = calendar.colors["bg_cur_day"];
						}
					}
...
document.writeln('<div id="__calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:210px;height:210px;">');
...
__ci.document.writeln('select.month {width:70px;}');
